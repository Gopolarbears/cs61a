...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def topicwords_in_para(para):
        para = split(lower(remove_punctuation(para)))
        for topicword in topic:
            for paraword in para:
                if topicword == paraword:
                    return True
        return False
    return topicwords_in_para
    # END PROBLEM 2
...
def wpm(typed, elapsed):
    ...
    return (character_counter) / 5 * 60 / elapsed
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for current_word in valid_words:
        if user_word == current_word:
            return current_word
    lowest_index = 0
    lowest_diff = diff_function(user_word, valid_words[0],limit)
    for current_index in range(len(valid_words)):
        if diff_function(user_word, valid_words[current_index], limit) < lowest_diff:
            lowest_index = current_index
            lowest_diff = diff_function(user_word, valid_words[current_index], limit)
...
def pawssible_patches(start, goal, limit):
    ...
    if start == "" and goal == "": # Fill in the condition
        ...
        substitute_diff = pawssible_patches(goal[0] + start[1:], goal, limit - 1)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, prompt, user_id, send):
    ...
    current_index = 0
    counter = 0
    while current_index < min(len(typed), len(prompt)) and typed[current_index] == prompt[current_index] :
        counter += 1
        current_index += 1
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    game_times = [[0] * len(words) for _ in times_per_player]
    for player_index in range(len(times_per_player)):
        for word_index in range(len(times_per_player[player_index]) - 1):
            game_times[player_index][word_index] = times_per_player[player_index][word_index + 1] - times_per_player[player_index][word_index]
...
def fastest_words(game):
    ...
    for word_index in word_indices:
        ...
        fastest_list[fastest_player_index].append(word_at(game, word_index))
