class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, name, price, fund=0, amount=0):
        self.name = name
        self.price = price
        self.fund = fund
        self.amount = amount
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add(rest_link, last_num):
        if rest_link.rest == ():
            rest_link.rest = Link(last_num)
        else:
            return add(rest_link.rest, last_num)
    def digits(n):
        if n // 10 != 0:
            num = n % 10
            s = store_digits(n//10)
...
def is_bst(t):
    ...
    def bst_min(tree1):
        if tree1.is_leaf():
            ...
            return min(tree1.label, min((bst_min(tree2) for tree2 in tree1.branches)))
    def bst_max(tree3):
        if tree3.is_leaf():
            return tree3.label
        else:
            return max((tree3.label), max(bst_max(tree4) for tree4 in tree3.branches))
    def bst(t):
        if t.is_leaf():
            return True
        if len(t.branches) == 1:
...
def preorder(t):
    ...
    if t.is_leaf():
        ...
        for tree1 in t.branches:
            lst.extend(preorder(tree1))
    return lst
...
def path_yielder(t, value):
    ...
    for tree2 in t.branches:
        for tree3 in path_yielder(tree2, value):
            # t_path.extend(tree3)
            # yield t_path
