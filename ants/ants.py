...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        for _ in range(self.min_range):
            if enemy_place.entrance is not beehive:
                enemy_place = enemy_place.entrance
                i_range += 1
        while not enemy_place.bees and enemy_place.entrance is not beehive and i_range < self.max_range:
            i_range += 1
            enemy_place = enemy_place.entrance
        return rANTdom_else_none(enemy_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        pre_place = self.place
        Ant.reduce_armor(self, amount)
        bees = pre_place.bees[:]
        for i in reversed(range(len(bees))):
            pre_place.bees[i].reduce_armor(amount)
        if self.place == None:
            bees1 = pre_place.bees[:]
            for s in reversed(range(len(bees1))):
                pre_place.bees[s].reduce_armor(self.damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee1 = random.choice(bee)
        while bee1.armor != 0:
            Insect.reduce_armor(bee1, 1)
        # END Problem 6
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.amount = QueenAnt.amount
        QueenAnt.amount = False
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.amount:
            ...
            while buff_place:
                if buff_place.ant and not hasattr(buff_place.ant, 'buffed'):
                    buff_place.ant.damage *= 2
                    buff_place.ant.buffed = True
    ...
    def remove_from(self, place):
        if self.amount != True:
            print('DEBUG: result is', QueenAnt.amount)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 5
        lazer_position = self.place
        dict ={}
        distance = 0
        while lazer_position != beehive:
            if lazer_position.ant and lazer_position.ant != self:
                dict[lazer_position.ant] = distance
            if lazer_position.bees:
                for bee in lazer_position.bees:
                    dict[bee] = distance
            lazer_position = lazer_position.entrance
            distance += 1
        return dict
        # END Problem Optional 5
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        goal_damage = self.damage - 0.2 * distance - 0.05 * self.insects_shot
        if goal_damage > 0:
            return goal_damage
        else:
            return 0
