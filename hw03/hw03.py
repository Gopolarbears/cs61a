...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m))):
            return balanced(end(left(m))) and balanced(end(right(m))) and True
        return False
...
def quadratic(x, a, b, c):
    ...
    l, u, e = map(f, (lower_bound(x), upper_bound(x), extre))
    if extre >= lower_bound(x) and extre <= upper_bound(x):
