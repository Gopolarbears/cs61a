def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + one_list for one_list in nested_list]
...
class Keyboard:
    ...
    def __init__(self, *args):
        ...
        for s in range(len(args)):
            self.buttons[args[s].pos] = args[s]
...
def insert(link, value, index):
    ...
    while index >= 0 and link is not ():
        if index == 0 and link.rest is ():
            link.rest = Link(value)
            return 
        elif index == 0 and link.rest is not ():
            link.first, link.rest = value, Link(link.first, link.rest)
            return 
        index -= 1
        link = link.rest
        if link is() and index > -1:
            print('debug')
            raise IndexError
