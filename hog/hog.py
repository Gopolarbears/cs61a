...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    num = player_score
    while not(player_score % num == 0 and opponent_score % num ==0):
        num -= 1
    if num >= 10 and opponent_score >= 10:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while s0 < goal and s1 < goal:
        num_rolls0 = strategy0(s0, s1)
        s0 = s0 + take_turn(num_rolls0, s1, dice)
        say = say(s0, s1)
        while s0 < goal and extra_turn(s0, s1):
            num_rolls0 = strategy0(s0, s1)
            s0 = s0 + take_turn(num_rolls0, s1, dice)
            say = say(s0, s1)
        if s0 >= goal:
        ...
        num_rolls1 = strategy1(s1, s0)
        s1 = s1 + take_turn(num_rolls1, s0, dice)
        say = say(s0, s1)
        while s1 < goal and extra_turn(s1, s0):
            num_rolls1 = strategy1(s1, s0)
            s1 = s1 + take_turn(num_rolls1, s0, dice)
            say = say(s0, s1)
        if s1 > goal:
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_function(*args):
        total = 0
        count_num =trials_count
        while count_num:
            total += original_function(*args)
            count_num -= 1
        average_num = total / trials_count
        return average_num
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while count <= 10:
        ...
        current_score = averaged_dice(count, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if bacon_strategy(score, opponent_score, cutoff, num_rolls) == 0 or pig_pass(score + free_bacon(opponent_score), opponent_score) or swine_align(score + free_bacon(opponent_score), opponent_score):
        return 0
    else:
        return num_rolls  # Replace this statement
